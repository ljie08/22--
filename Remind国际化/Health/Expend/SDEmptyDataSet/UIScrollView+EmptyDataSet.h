...
/**
 要求用于指定按钮状态的标题中的数据源。该数据集默认使用一个固定的字体样式，如果没有属性设置。如果你想要一个不同的字体样式，返回一个字符串归因。
 @Param scrollView 滚动型子类对象通知的数据源。 
 @Param state 使用指定标题的状态。的可能值中UIControlState进行说明。 
 @return 一个字符串归结为数据集按钮标题，结合字体，文本颜色，文本段落样式等。
 */
