...
- (void)flickerAnimation:(NSTimer *)timer{
    ...
    if([self.flickerNumber intValue] >= [timer.userInfo[EndNumberKey] intValue]){
        [self finalString:timer.userInfo[ResultNumberKey] stringFormat:formatStr andFormatter:self.flickerNumberFormatter];
        self.text = [NSString stringWithFormat:@"￥%@",[NSString reservePointCount:[(NSNumber*)(timer.userInfo[ResultNumberKey]) doubleValue] WithCount:2]];
        if(timer.userInfo[AttributeKey]){
            [self attributedHandler:timer.userInfo[AttributeKey]];
        }
        [timer invalidate]; /////
...
- (void)floatNumberHandler:(NSTimer *)timer andMultiple:(int)multiple{
   ...
   self.text = [self finalString:@([self.flickerNumber floatValue]/multiple) stringFormat:formatStr andFormatter:self.flickerNumberFormatter];
    ...
    if (![[NSUserDefaults standardUserDefaults]boolForKey:@"flickNumber"]) {
    ...
    if([self.flickerNumber intValue] >= [timer.userInfo[EndNumberKey] intValue]){
        [self finalString:timer.userInfo[ResultNumberKey] stringFormat:formatStr andFormatter:self.flickerNumberFormatter];
        self.text = [NSString reservePointCount:[(NSNumber*)(timer.userInfo[ResultNumberKey]) doubleValue] WithCount:2];
        if (![[NSUserDefaults standardUserDefaults]boolForKey:@"flickNumber"]) {
