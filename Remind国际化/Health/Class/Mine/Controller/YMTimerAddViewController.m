...
//#define YMLocalNotificationRemind   @"localnotificationRemind"
...
@interface YMTimerAddViewController ()<YMPickerViewDelegate,YMRemindTitleViewControllerDeleagate,YMBellViewControllerDelegate>

@property (nonatomic, strong) YMPickerView *pickerView;
@property (nonatomic, strong) NSMutableArray      *titleArr;
@property (nonatomic, strong) NSArray             *timeCountArr;//每次提醒次数
...
@property (nonatomic, copy)   NSString     *dosageStr;//剂量
...
@implementation YMTimerAddViewController
...
//    self.timeCountArr = [NSArray arrayWithObjects:@"第一次",@"第二次",@"第三次",@"第四次",@"第五次",@"第六次", nil];
//    self.titleArr = [NSMutableArray arrayWithObjects:@[@"闹钟开关",@"标题"],@[@"剂量",@"语音播报"],@[@"震动",@"铃声"],@[@"周期"],nil];
//    self.timeArr = [NSMutableArray array];
//    self.selectedDaysArr = [NSMutableArray array];
...
//    
//    [self.timeArr addObject:@"上午09:00"];
//    [self.timeArr addObject:@"下午03:00"];
//    [self.timeArr addObject:@"下午09:00"];
...
- (void)setRemindType:(TimerRemindType)remindType{
    ...
    if (_remindType == kTimerRemindDrug) {
        self.navTitle =LOCALIZED( @"吃药");
        self.isDrug = YES;
    }else if (_remindType == kTimerRemindDrinkWater){
    
        self.navTitle = LOCALIZED(@"喝水");
...
- (void)setContent:(YMTimerRemindMode *)content{
    ...
    if (_content) {//修改
        ...
        if ([tArr count]) {
            [self.timeArr addObjectsFromArray:tArr];
            [self.timeArr removeObject:@""];
        }
//        for (NSString *tStr in tArr) {
//            if (tStr.length > 0) {
//                [self.timeArr addObject:[self getTimeStrWithRemindStr:tStr]];
//            }
//        }
        [self.titleArr insertObject:[self getArrWithSourceArray:self.timeCountArr andCount:self.timeArr.count] atIndex:1];
        if (_content.medicineInfo.length > 0) {
            NSMutableString *mStr = [NSMutableString stringWithString:_content.medicineInfo];
            [mStr replaceCharactersInRange:NSMakeRange(0, _content.medicineInfo.length) withString:@""];
            self.dosageStr = mStr;
        }
        self.isOpenVoice = [_content.remindVoice boolValue];
...
- (NSMutableArray *)titleArr{
    ...
    if (!_titleArr) {
        if (self.remindType == kTimerRemindPluse) {
...
//            _titleArr = [NSMutableArray arrayWithObjects:@[@"闹钟开关",@"标题"],@[@"震动"],@[@"周期"],nil];
            ...
            NSString *str = self.isDrug ? @"剂量" : @"容量" ;
//            _titleArr = [NSMutableArray arrayWithObjects:@[@"闹钟开关",@"标题"],@[str,@"语音播报"],@[@"震动",@"铃声"],@[@"周期"],nil];
            _titleArr = [NSMutableArray arrayWithObjects:@[LOCALIZED(@"闹钟开关"),LOCALIZED(@"标题")],@[LOCALIZED(str)],@[LOCALIZED(@"震动")],@[LOCALIZED(@"周期")],nil];

//            _titleArr = [NSMutableArray arrayWithObjects:@[@"闹钟开关",@"标题"],@[str],@[@"震动"],@[@"周期"],nil];
...
- (void)rightItemDidClicked{
    ...
    if (isUploadData) {
        ...
        content.remindVoice = [NSString stringWithFormat:@"%d",self.isOpenVoice];
        NSMutableString *mStr = [NSMutableString string];
        if (self.dosageStr.length > 0) {
            [mStr appendString:self.dosageStr];
            
            NSInteger index = self.dosageStr.length - 1;
            if ([self.dosageStr hasPrefix:@"毫升"]) {
                index = self.dosageStr.length - 2;
            }
            [mStr insertString:@"," atIndex:index];
        }
        
        content.medicineInfo = mStr;
...
- (NSString *)getDayStrWithArray:(NSArray *)array{
    ...
    for (NSInteger i = 0; i < array.count; i++) {
        ...
        if ([self isSelectedDayWithTag:107]) {//工作日
            ...
            return mStr;
        }else if ([self isSelectedDayWithTag:108]) {//周日
        ...
        if ([sArr indexOfObject:day] == NSNotFound) {
            if (btn.tag >= 104 && btn.tag <= 106) {//六 日
                ...
                if ([self isSelectedDayWithTag:107]) {//取消工作日按钮
                    ...
                    [self.tableView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:2 inSection:4]] withRowAnimation:UITableViewRowAnimationNone];
                }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 1) {//定时
        if (indexPath.row < arr.count) {
            ...
            UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(lefaSpace, 49, SCREEN_WIDTH - 2* lefaSpace, 1)];
...
//            [cell.contentView addSubview:lineView];
    ...
    if (indexPath.section == 4 && indexPath.row != 0) {//一周
...
//    }
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            cell.switchView.hidden = YES;
            if (self.dosageStr.length > 0) {
                if (self.remindType == kTimerRemindDrug) {
                    cell.cLabel.attributedText = [self getmAttStrWithString:self.dosageStr];
                }else{
                    cell.cLabel.text = self.dosageStr;
                }
            }
//            cell.cLabel.text = @"每次2片";
            cell.rightImgName = @"箭头-右";
        }else{
            
            cell.cLabel.hidden = YES;
            cell.switchView.hidden = NO;
            cell.switchView.on = self.isOpenVoice;
            [cell.switchView addTarget:self action:@selector(voiceSwitchDidClicked:) forControlEvents:UIControlEventValueChanged];
        }
...
//                    [self.timeArr addObject:[[YMClockManager shareManager] getCurrentTime]];
//                }else{//获取上一个提醒时间
//                
//                }
        ...
        self.pickerView.dataType = self.isDrug ? UserDataTypeDosage : UserDataTypeCup;
        [self.pickerView  show];
...
//- (NSString *)getIncreaseTimeWithLastTime:(NSString *)lastTime{
//
//    NSMutableString *mStr = [NSMutableString string];
//
//    NSArray *tAtt = [lastTime componentsSeparatedByString:@":"];
//    
//    
//    
//    return mStr;
...
- (void)selectedComponentInPickerViewWithcontent:(NSString *)content{
    ...
    if (self.pickerView.dataType == UserDataTypeDosage) {
        if (content.length >0) {
            self.dosageStr = content;
            YMClockCell *cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:2]];
            cell.cLabel.attributedText = [self getmAttStrWithString:self.dosageStr];
        }
        
    }else if (self.pickerView.dataType == UserDataTypeCup) {
        if (content.length >0) {
            self.dosageStr = content;
            YMClockCell *cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:2]];
            cell.cLabel.text = content;
        }
        
    }else if (self.pickerView.dataType == UserDataTypeDate){
        
        NSString *tStr = [[YMClockManager shareManager] getTimeWith12HourTimeStr:content];
        [self.timeArr replaceObjectAtIndex:self.selectIndexPath.row withObject:tStr];
        YMClockCell *cell = [self.tableView cellForRowAtIndexPath:self.selectIndexPath];
        cell.cLabel.text = tStr;
    }
}

- (NSMutableAttributedString *)getmAttStrWithString:(NSString *)content{
    NSString *str = [NSString stringWithFormat:@"每次%@",content];

    NSMutableAttributedString *mAttStr = [[NSMutableAttributedString alloc] initWithString:str];
    [mAttStr addAttribute:NSForegroundColorAttributeName value:COLOR_000 range:NSMakeRange(2, str.length - 2)];
    return mAttStr;
}
...
//                    key = [NSString stringWithFormat:@"%@%@周%ld%@",self.navTitle,YMDays,weekDay,self.timeCountArr[i]];
//                    [[YMClockManager shareManager] cancelLocalNotificationWithKey:key];
        ...
        if (_content.remindTime.length > 0) {
            ...
            for (NSInteger i = 0; i < timeArr.count; i ++) {
                ...
                if (str.length > 0) {
//                    key = [NSString stringWithFormat:@"%@%@%@",self.navTitle,YMOnceDay,self.timeCountArr[i]];
//                    [[YMClockManager shareManager] cancelLocalNotificationWithKey:key];
